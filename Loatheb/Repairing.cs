...
public class Repairing
{
	...
	public bool NeedsRepairingTool()
	{
		...
		return _openCv.IsMatching(_images.ToolNeedsRepairing);
	}
	...
	public bool PetFunctionShowing()
	{
		...
		return _openCv.IsMatching(_images.PetFunction);
	}

	public bool OpenToolRepair()
	{
		Console.WriteLine("Checking for tool repair button");
		var (matches, locations) = _openCv.IsMatchingWhere(_images.RepairToolBtn);
		if (matches)
		{
	...
	public bool RepairToolActive()
	{
		...
		return _openCv.IsMatching(_images.RepairToolActive, 0.95) && _openCv.IsMatching(_images.RepairAll);
	}

	public bool ClickRepairAll()
	{
		Console.WriteLine("Checking Repair All button");
		var (result, locations) = _openCv.IsMatchingWhere(_images.RepairAll);
		if (result)
		{
			_mouseCtrl.Move(locations);
			Thread.Sleep(100);
	...
	public bool ClickRepairAndOk()
	{
		for (int i = 0; i < 5; i++)
		{
			...
			if (_openCv.IsMatching(_images.RepairAll))
			{
				return ClickOk();
			}
			Thread.Sleep(400);
	...
	public bool ClickOk()
	{
		var (result, locations) = _openCv.IsMatchingWhere(_images.OkBtn);
